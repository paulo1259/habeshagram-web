...
export async function uploadShortVideo(input: {
  file: File;
  userId: string;
  shortId: string;
}): Promise<{ url: string; storagePath: string }> {
  if (!firebaseStorage) {
    throw new Error("Firebase Storage is not configured for short video uploads.");
  }

  const extension = getFileExtension(input.file);
  const path = `shorts/${input.userId}/${input.shortId}/video-${Date.now()}.${extension}`;
  ...
  try {
    return await uploadStorageFile({
      ...
      file: input.file,
      contentType: input.file.type || undefined,
      uploadTimeoutMessage: "Timed out while uploading your short video.",
      urlTimeoutMessage: "Timed out while preparing your uploaded short video."
    });
  } catch (error) {
    const code = typeof error === "object" && error && "code" in error ? String(error.code) : "";
    ...
    switch (code) {
      case "storage/unauthorized":
        throw new Error("Your Firebase Storage rules blocked this short upload. Update your storage permissions and try again.");
      case "storage/canceled":
        throw new Error("Short video upload was canceled.");
      case "storage/unknown":
        throw new Error("Storage could not finish the short upload. Please try again.");
      default:
        throw new Error(error instanceof Error ? error.message : "Unable to upload your short video.");
    }
  }
}

export async function uploadShortThumbnail(input: {
  blob: Blob;
  userId: string;
  shortId: string;
}): Promise<{ url: string; storagePath: string }> {
  if (!firebaseStorage) {
    throw new Error("Firebase Storage is not configured for short thumbnails.");
  }

  const path = `shorts/${input.userId}/${input.shortId}/thumbnail-${Date.now()}.jpg`;
  ...
  try {
    return await uploadStorageFile({
      ...
      uploadTimeoutMessage: "Timed out while uploading your short thumbnail.",
      urlTimeoutMessage: "Timed out while preparing your short thumbnail."
    });
  } catch (error) {
    const code = typeof error === "object" && error && "code" in error ? String(error.code) : "";
    ...
    switch (code) {
      case "storage/unauthorized":
        throw new Error("Your Firebase Storage rules blocked this thumbnail upload. Update your storage permissions and try again.");
      case "storage/canceled":
        throw new Error("Short thumbnail upload was canceled.");
      default:
        throw new Error(error instanceof Error ? error.message : "Unable to upload your short thumbnail.");
    }
  }
}
