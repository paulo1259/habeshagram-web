...
import { calculateHotPosts, calculateTrendingHashtags } from "@/services/trending-service";
          ...
          <div className="mt-3 space-y-2">
            {hotPosts.length ? hotPosts.slice(0, compact ? 2 : 3).map(({ post, score }) => (
              <Link
                key={post.id}
                href={`/posts/${post.id}`}
                className="block rounded-[20px] border border-brand-100/80 bg-card/92 px-4 py-3 transition hover:-translate-y-0.5 hover:shadow-sm"
              >
