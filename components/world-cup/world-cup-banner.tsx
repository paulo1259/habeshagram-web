        ...
        <div className="flex-1 space-y-2">
          ...
          {isLive ? (
            <div className="flex items-center gap-2">
              ...
              <p className="text-lg font-black tracking-tight">Tournament trust mode is live</p>
            </div>
          ) : (
            <p className="text-lg font-black tracking-tight tabular-nums">
              Opens in {countdown.days}d {String(countdown.hours).padStart(2, "0")}h{" "}
              {String(countdown.minutes).padStart(2, "0")}m {String(countdown.seconds).padStart(2, "0")}s
            </p>
          )}

          {nextMatch && teamA && teamB ? (
            <div className="flex flex-wrap items-center gap-1.5 text-xs text-white/82">
              <span className="font-semibold">
                {hasFav ? "Your team:" : "Next verified fixture:"}
              </span>
